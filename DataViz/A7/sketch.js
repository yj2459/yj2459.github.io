// hour = diameter of circles
// minute = lenght of sticks
// second = vector
...
function draw() {    
  ...
  if (systems.length==0) {
    ...
    textAlign(CENTER);
    textSize(32);
    text("click mouse to check TIME", width/2, height/2);
  }
}
...
var Particle = function(position) {
  var s = second();
  this.acceleration = createVector(0, map(s, 0, 60, -1, 1));
...
Particle.prototype.display = function () {
  ...
  ellipse(this.position.x, this.position.y, map(h, 0, 12, 12, 24), map(h, 0, 12, 12, 24));
};
